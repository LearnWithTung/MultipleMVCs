...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let feedVC = navigationController.viewControllers.first as! FeedViewController
        fetchCellControllers { controllers in
            feedVC.dataSource = controllers
        }
        
        window?.rootViewController = navigationController
    }
    
    private func fetchCellControllers(completion: @escaping ([ImageCellController]) -> Void){
